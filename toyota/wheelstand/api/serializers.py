...
from ..models import Models, Trim, ModelsShown, Accessories, Location, Colours, Interiors, Gallery, ColourGallery, \
	Dealers, DealerProvince, ModelsShownInteriors, ModelsShownGallery, AllAccessories, AccessoriesShown, DealerCity, \
	TestDrive
...
class ModelsSerializer(serializers.ModelSerializer):
	class Meta:
		model = Models
		fields = ('id', 'name', 'internal_id', 'year', 'subhead', 'url_logo', 'link_logo', 'url', 'link', 'disclaimer',
		          'base_price', 'freight_DPI', 'gallery', 'lease_APR_from', 'lease_APR_for', 'lease_payment_from',
		          'down_payment', 'accessory', 'price_override', 'surchargeFees', 'dealerFees')
		extra_kwargs = {
			'url': {
				'required': False,
				#                    'allow_blank': True,
...
class ColourGallerySerializer(serializers.ModelSerializer):
	class Meta:
		model = ColourGallery
		fields = ('name', 'hexcode', 'url', 'link', 'md5', 'status_changed')
...
class TrimSerializer(serializers.ModelSerializer):
	...
	class Meta:
		model = Trim
		fields = (
		'id', 'order', 'model', 'name', 'internal_id', 'url', 'link', 'features', 'price', 'interiors', 'trim_gallery')
		#        depth = 1
		extra_kwargs = {
			'url': {
				'required': False,
...
class ModelsShownGallerySerializer(serializers.ModelSerializer):
	class Meta:
		...
		fields = ('id', 'name', 'hexcode', 'url', 'link', 'md5', 'status_changed', 'models_shown_gallery')
...
class ModelsShownSerializer(serializers.ModelSerializer):
	...
	class Meta:
		model = ModelsShown
		fields = (
		'id', 'vehicle', 'trim', 'colour', 'legal_disclaimer', 'standard_features', 'price_override', 'starsafety',
		'TSS', 'IIHS', 'series_code', 'fuel_city', 'fuel_highway', 'fuel_combined', 'horsepower', 'engine_displacement',
		'engine_description', 'seats', 'airbags', 'torque', 'towing_capacity', 'accessory', 'price_override_accessory',
