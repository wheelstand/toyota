...
from .models import Models, Trim, ModelsShown, Accessories, Location, Colours, Interiors, Gallery, ColourGallery, \
	Dealers, DealerCity, DealerProvince, ModelsShownInteriors, ModelsShownGallery, AllAccessories, AccessoriesShown, \
	TestDrive
...
class ModelsAdmin(admin.ModelAdmin):
	...
	fieldsets = (
		(_('General'), {
			'fields': (('name_en', 'name_fr'), 'internal_id', 'year', 'subhead_en', 'subhead_fr',
		...
		(_('Special accessories'), {
			'fields': ('accessory', 'price_override', 'surchargeFees', 'dealerFees')
		}),
	)
...
class TrimAdmin(admin.ModelAdmin):
	...
	fieldsets = (
		(_('General'), {
			'fields': ('model', ('name_en', 'name_fr'), 'order', 'internal_id'),
		}),
		(_('Details'), {
...
class ModelsShownAdmin(admin.ModelAdmin):
	...
	fieldsets = (
		(_('General'), {
			'fields': ('vehicle', 'trim', 'colour', ('legal_disclaimer_en', 'legal_disclaimer_fr'),
			           ('standard_features_en', 'standard_features_fr'), 'price_override', 'starsafety', 'TSS', 'IIHS',
			           'series_code'),
		}),
		(_('Highlights'), {
			'fields': ('fuel_city', 'fuel_highway', 'fuel_combined', 'horsepower', 'engine_displacement',
			           ('engine_description_en', 'engine_description_fr'), 'seats', 'airbags', 'torque',
			           'towing_capacity'),
		}),
...
class ColoursAdmin(admin.ModelAdmin):
	list_display = ('name_en', 'name_fr', 'hexcode')
	fieldsets = (
		(_('General'), {
